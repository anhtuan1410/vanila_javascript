...
var inputMask = {
  ...
  createShell : function (t) {
    var wrap = document.createElement('span'),
        ...
        pTxt = t.getAttribute('placeholderMask'),
        placeholderMask = document.createTextNode(pTxt);

    t.setAttribute('maxlength', placeholderMask.length);
    t.setAttribute('data-placeholderMask', pTxt);
    //t.removeAttribute('placeholderMask');
    ...
    mask.appendChild(placeholderMask);
  ...
  setValueOfMask : function (e) {
    var value = e.target.value,
        placeholderMask = e.target.getAttribute('data-placeholderMask');

    return "<i>" + value + "</i>" + placeholderMask.substr(value.length);
  },
  ...
  handleCurrentValue : function (e) {
    var isCharsetPresent = e.target.getAttribute('data-charset'),
        placeholderMask = isCharsetPresent || e.target.getAttribute('data-placeholderMask'),
        value = e.target.value, l = placeholderMask.length, newValue = '',
    ...
    for (i = 0, j = 0; i < l; i++) {
        ...
        matchesNumber = this.opts.mNum.indexOf(placeholderMask[i]) >= 0;
        matchesLetter = this.opts.mChar.indexOf(placeholderMask[i]) >= 0;
  ...
  validateProgress : function (e, value) {
    ...
    //convert to months
    if (l == 1 && placeholderMask.toUpperCase().substr(0,2) == 'MM') {
      if(value > 1 && value < 10) {
        value = '0' + value;
      }
      return value;
